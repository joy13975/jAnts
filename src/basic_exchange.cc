            ...
            double startTime)
{
    ...
    while (true)
    {
        const float oldScore = bestScore;
        ...
        for (int i = 0; i < dim - 1; i++)
        {
            for (int j = i + 1; j < dim - 1; j++)
            {
                ...
                const float c1 = Route(nodes, sol, vcap).calcScoreWithCache(realDists);
                Route newRoute1 = Route(nodes, newSol1, vcap);
                const float c2 = newRoute1.calcScoreWithCache(realDists);
                ...
                const float c3 = newRoute2.calcScoreWithCache(realDists);
