...
public class Graph extends JPanel {
    ...
    private void generateColours() {
        ...
        for (int i = 0; i < trucks.size(); i++) {
            truckColours.add(new Color(g.nextInt(256),
                                       g.nextInt(256),
                                       g.nextInt(256)));
            g.nextInt(555);
        }
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Draw depot
        drawNode(g2d, DEPOT_COLOUR, getLocation(1), 5);

        // Draw customers
        for (int i = 2; i < CVRPData.NUM_NODES; i++) {
            drawNode(g2d, POINT_COLOUR, getLocation(i), (float) CVRPData.getDemand(i) / 10);
        }
    }
    ...
    protected void drawNode(Graphics2D g, Color c, Point p, float radius) {
